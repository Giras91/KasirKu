...
public class SettingDetailActivity extends AppCompatActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (savedInstanceState == null) {
			...
			arguments.putString(SettingDetailFragment.ARG_ITEM_ID, getIntent()
					.getStringExtra(SettingDetailFragment.ARG_ITEM_ID));
			SettingDetailFragment fragment = new SettingDetailFragment();
			fragment.setArguments(arguments);
			getSupportFragmentManager().beginTransaction()
					.add(R.id.setting_detail_container, fragment).commit();
		}
	}
